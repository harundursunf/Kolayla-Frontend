...
const Personal = () => {
  ...
  const getUserIdFromToken = (token) => {
    ...
    try {
      ...
      return decodedPayload["http://schemas.xmlsoap.org/ws/2005/05/identity/claims/nameidentifier"] || decodedPayload.sub;
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-white py-12 px-4 sm:px-6 lg:px-8 mt-[50px]">
      <div className="max-w-6xl mx-auto bg-white p-8 rounded-2xl shadow-3xl space-y-10 border border-gray-100">

        <div className="text-center">
          <h2 className="text-5xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-purple-700 to-pink-700 drop-shadow-md">
        ...
        <div className="border-t-2 border-blue-200 pt-8 bg-blue-50/30 p-6 rounded-lg shadow-inner">
          ...
          <div className="flex flex-col md:flex-row items-center space-y-4 md:space-y-0 md:space-x-4 w-full">
            ...
            <label className="flex items-center cursor-pointer space-x-2 px-3 py-2 bg-white rounded-lg shadow-sm border border-gray-200 hover:border-red-400 transition duration-200 ease-in-out">
              <input
                type="checkbox"
                checked={isPriority}
        ...
        <div className="border-t-2 border-purple-200 pt-8">
          ...
          <div className="mb-6 flex items-center space-x-3">
             <IoSearchOutline className="h-6 w-6 text-gray-500" />
            <input
              type="text"
              value={searchTerm}
          ...
          {!loading && filteredAndSortedGoals.length === 0 ? (
            <div className="text-center py-10 text-gray-600 italic text-xl">
                <IoRocketOutline className="h-12 w-12 mx-auto text-purple-500 mb-4" />
              <p>
                {searchTerm
              ...
              {filteredAndSortedGoals.map((goal) => (
                ...
                >
                   ...
                   <div className="text-sm text-gray-500">
                     Oluşturuldu: {new Date(goal.createdDate).toLocaleDateString('tr-TR', { year: 'numeric', month: 'long', day: 'numeric' })}
                   </div>
                   ...
                   <div className="flex justify-between items-center pt-4 border-t border-gray-200">
                     ...
                     >
                       ...
                       <span>{goal.isCompleted ? 'Tamamlandı' : 'Tamamlandı'}</span>
                     </button>
                     ...
                     <button
                       ...
                       className="text-red-500 hover:text-red-700 font-medium py-1 px-3 rounded-full transition duration-200 ease-in-out transform hover:scale-110 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-opacity-50"
                        title="Hedefi Kalıcı Olarak Sil"
                         disabled={loading}
                     >
                        <IoTrashBin className="h-5 w-5 inline-block mr-1" />
                        Sil
                     </button>
                   </div>
                 </div>
